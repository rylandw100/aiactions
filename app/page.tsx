...
export default function Home() {
 ...
 Use their name, role, team, and start date. Keep it friendly and concise.`);
  ...
  useEffect(() => {
    function updateVariablePopoverPosition() {
      if (showVariablePicker) {
        ...
        // If opened from cursor position (typing "{{"), use cursor position
        if (cursorPosition.top > 0 && cursorPosition.left > 0) {
          const spaceBelow = window.innerHeight - cursorPosition.top;
          const spaceAbove = cursorPosition.top;
          const spaceRight = window.innerWidth - cursorPosition.left;
          const spaceLeft = cursorPosition.left;
          
          let top: number;
          let left: number;
          
          // Position popover below the line where cursor is
          // cursorPosition.top is already the bottom of the line (from getBoundingClientRect().bottom)
          const verticalSpacing = 8; // Small spacing below the line
          
          // Always try to position below the current line
          if (spaceBelow >= popoverHeight + verticalSpacing) {
            // Position below the line
            top = cursorPosition.top + verticalSpacing;
          } else if (spaceAbove >= popoverHeight + verticalSpacing) {
            // Not enough space below, position above the line
            top = cursorPosition.top - popoverHeight - verticalSpacing;
          } else {
            // Not enough space either way, position below but adjust to fit
            top = Math.max(spacing, window.innerHeight - popoverHeight - spacing);
          }
          
          // Align horizontally with the start of the line (left edge of textarea)
          // This keeps it aligned with the text and doesn't block the cursor
          const textareaId = pickerContext === "sms" ? 'sms-message' : 'prompt-textarea';
          const textarea = document.getElementById(textareaId);
          if (textarea) {
            const textareaRect = textarea.getBoundingClientRect();
            // Align with left edge of textarea (where text starts)
            ...
            if (spaceRight >= popoverWidth) {
              ...
              left = cursorPosition.left - popoverWidth;
            } else {
              left = spacing;
            }
  ...
  }, [showVariablePicker, pickerContext, cursorPosition]);
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#f9f7f6]">
                      ...
                      onChange={(text) => {
                        ...
                        if (textarea) {
                          ...
                          if (textBeforeCursor.endsWith("{{")) {
                            ...
                            // Get the actual cursor position using selection range
                            const selection = window.getSelection();
                            const range = selection?.rangeCount ? selection.getRangeAt(0) : null;
                            ...
                            if (range && textarea.contains(range.commonAncestorContainer)) {
                              ...
                              // Position is the bottom of the cursor (end of current line)
                              setCursorPosition({
                                top: cursorRect.bottom, // Bottom of the line where cursor is
                                left: cursorRect.left, // Left edge of cursor
                              });
                            } else {
                              // Fallback: calculate from textarea position
                              const textareaRect = textarea.getBoundingClientRect();
                              const textBeforeCursorLines = textBeforeCursor.split('\n');
                              const currentLine = textBeforeCursorLines.length - 1;
                              const lineHeight = 24;
                              const scrollTop = (textarea as HTMLElement).scrollTop || 0;
                              
                              setCursorPosition({
                                top: textareaRect.top + (currentLine * lineHeight) - scrollTop + lineHeight,
            ...
            {selectedNode === "sms" && (
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h3 className="text-base font-medium text-black mb-4">Details</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-black mb-2">
                  ...
                  <div>
                    <h3 className="text-base font-medium text-black mb-4">Settings</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="flex items-center gap-2 text-sm font-medium text-black mb-2">
                          Time zone <span className="text-[#c3402c]">*</span>
                          <HelpCircle className="size-4 text-[#8c8888]" />
                        </label>
                        <div className="relative">
                          <Input
      ...
      {showVariablePicker && (pickerContext === "sms" || pickerContext === "aiPrompt") && (
        <div 
          className="variable-popover fixed z-50 bg-white border border-[#e0dede] rounded-lg shadow-lg w-[400px] max-h-[400px] flex flex-col"
          style={{
